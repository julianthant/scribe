...
Provides REST API endpoints for email operations using Microsoft Graph API.
This module handles:
- GET /mail/folders: List all mail folders with hierarchy
- POST /mail/folders: Create new mail folders
- GET /mail/folders/{folder_id}/messages: List messages in a folder
- GET /mail/messages/{message_id}: Get specific message details
- PUT /mail/messages/{message_id}: Update message properties
- POST /mail/messages/move: Move messages between folders
- POST /mail/messages/search: Search messages with filters
- GET /mail/messages/{message_id}/attachments: List message attachments
- GET /mail/attachments/{attachment_id}/download: Download attachments
- POST /mail/voice/organize: Organize voice attachments
- GET /mail/folders/{folder_id}/statistics: Get folder statistics
...
from app.models.MailModel import (
    ...
    SearchMessagesRequest, OrganizeVoiceRequest, OrganizeVoiceResponse,
    VoiceAttachment, FolderStatistics, Attachment
)
...
@router.get("/voice-attachments", response_model=List[VoiceAttachment])
...
@router.get("/messages/{message_id}/voice-attachments", response_model=List[VoiceAttachment])
...
@router.get("/voice-attachments/{message_id}/{attachment_id}/metadata")
...
@router.get("/voice-attachments/{message_id}/{attachment_id}/download")
...
@router.post("/voice-attachments/store/{message_id}/{attachment_id}")
...
@router.get("/voice-attachments/stored")
...
@router.get("/voice-attachments/blob/{blob_name}")
...
@router.delete("/voice-attachments/blob/{blob_name}")
...
@router.get("/voice-attachments/storage-statistics")
...
@router.post("/voice-attachments/cleanup")
